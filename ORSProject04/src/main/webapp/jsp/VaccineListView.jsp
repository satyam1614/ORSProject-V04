<%@page import="com.sunilos.p4.bean.VaccineBean"%>
		...
		<form action="<%=ORSView.VACCINE_LIST_CTL%>" method="POST">
			...
			<div
				...
				<input type="text" name="vaccineName"
					...
					value="<%=ServletUtility.getParameter("vaccineName", request)%>">
			...
			<div class="table-responsive">
				<table class="table table-hover align-middle mb-0">
					...
					<tbody>
						<%
						while (it.hasNext()) {
							VaccineBean bean = it.next();
						%>
						<tr>
							<td><input type="checkbox" name="ids"
								value="<%=bean.getId()%>"></td>
							<td class="text-muted small"><%=index++%></td>
							<td class="fw-semibold"><%=bean.getVaccineName()%></td>
							<td><%=bean.getManufacturer()%></td>
							<td><%=bean.getExpiryDate()%></td>
							<td><a href="VaccineCtl?id=<%=bean.getId()%>"
