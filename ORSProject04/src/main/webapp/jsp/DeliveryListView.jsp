	...
	<div class="card border-0 shadow-sm rounded-4 overflow-hidden">
		...
		<div
			...
			<div class="d-flex gap-2">
				...
				</a> <a href="<%=ORSView.DELIVERY_CTL%>"
					class="btn btn-sm btn-light text-primary fw-semibold"> <i
					class="bi bi-cart me-1"></i> Add Delivery
				</a>
			</div>
		</div>
		...
		<form action="<%=ORSView.DELIVERY_LIST_CTL%>" method="POST">
			...
			<div
				...
				<input type="text" name="deliveryBoyName"
			...
			<div class="table-responsive">
				<table class="table table-hover align-middle mb-0">
					...
					<tbody>
						<%
						while (it.hasNext()) {
							DeliveryBean bean = it.next();
						%>
						<tr>
							<td><input type="checkbox" name="ids"
								value="<%=bean.getId()%>"></td>
							<td class="text-muted small"><%=index++%></td>
							<td class="fw-semibold"><%=bean.getDeliveryBoyName()%></td>
							<td><%=bean.getAddress()%></td>
							<td><%=bean.getDeliveryDate()%></td>
							<td><%=bean.getStatus()%></td>
							<td><a href="DeliveryCtl?id=<%=bean.getId()%>"
