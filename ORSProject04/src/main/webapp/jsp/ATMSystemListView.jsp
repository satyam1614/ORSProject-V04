...
Iterator<ATMSystemBean> it = list.iterator();
	...
	<div class="card border-0 shadow-sm rounded-4 overflow-hidden">
		...
		<div
			...
			<h5 class="mb-0 fw-bold">
				<i class="bi bi-cart me-2"></i> ATMSystem List
			</h5>
			<div class="d-flex gap-2">
				<a href="#" target="_blank"
					...
					class="bi bi-cart me-1"></i> Add ATMSystem
				</a>
			</div>
		</div>

		<form action="<%=ORSView.ATMSYSTEM_LIST_CTL%>" method="POST">
			...
			<div
				...
				<button type="submit" name="operation"
					...
					onclick="return confirm('Delete this ATMSystem?')">
			...
			<div class="table-responsive">
				<table class="table table-hover align-middle mb-0">
					...
					<tbody>
						<%
						while (it.hasNext()) {
							ATMSystemBean bean = it.next();
						%>
						<tr>
							<td><input type="checkbox" name="ids"
								value="<%=bean.getId()%>"></td>
							<td class="text-muted small"><%=index++%></td>
							<td class="fw-semibold"><%=bean.getBankName()%></td>
							<td><%=bean.getLocation()%></td>
							<td><%=bean.getCashAvailable()%></td>
