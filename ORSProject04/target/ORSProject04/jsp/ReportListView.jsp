<%@page import="com.sunilos.p4.bean.ReportBean"%>
			...
			<div class="table-responsive">
				<table class="table table-hover align-middle mb-0">
					...
					<tbody>
						<%
						while (it.hasNext()) {
							ReportBean bean = it.next();
						%>
						<tr>
							<td><input type="checkbox" name="ids"
								value="<%=bean.getId()%>"></td>
							<td class="text-muted small"><%=index++%></td>
							<td class="fw-semibold"><%=bean.getType()%></td>
							<td><%=bean.getGeneratedDate()%></td>
							<td><%=bean.getRemarks()%></td>
							<td><a href="ReportCtl?id=<%=bean.getId()%>"
